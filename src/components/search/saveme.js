            ...
            return(
                <div className='center'>
                    ...
                    {/* <p>{bookLoading=="empty"?'No Match Found': 'Here is your result'}</p> */}
                        {
                            bookLoading?
                            bookData.map(book => {return <EachBook image={book.volumeInfo.imageLinks.smallThumbnail}
                                                        ...
                                                        key={book.id}/>}):
                                                        <p>{keyEmpty2? <span>Type in what you want to search in the search box</span>:<span> {!noBook ? <span>Loading...</span> : <span>No Response recieved from the server</span>}</span>}</p>
                        }
                    </div> 
